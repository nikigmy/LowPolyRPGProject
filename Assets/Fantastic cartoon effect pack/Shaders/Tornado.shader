Shader "Tornado" {
    Properties {
		...
		_TwistScale("Reel vector", Vector) = (1, 0.1, 5, 0)
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
				...
				float height = v.vertex.y * _TwistScale.y;
				v.vertex.x += sin(_Time.y*_TwistScale.z + worldpos.y * _TwistScale.x) * height;
				v.vertex.z += sin(_Time.y*_TwistScale.z + worldpos.y * _TwistScale.x + 3.1415/2) * height;
				o.height = height;
