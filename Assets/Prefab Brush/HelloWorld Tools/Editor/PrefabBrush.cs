...
public class PrefabBrush : EditorWindow {
    ...
    private float opacityDelay = 0.2f;
    private float lastOpacityDelay = 0;
        ...
        if (placmentBrush && isOn)
        {
            ...
            if (Event.current.type == EventType.MouseDrag && Event.current.button == 0 && placmentBrush || Event.current.type == EventType.MouseDown && Event.current.button == 0 && placmentBrush)
            {
                ...
                if (selectedObject == null)
                {
                    ...
                    if(opacityValue < 1)
                    {
                        if(lastOpacityDelay == 0)
                        {
                            lastOpacityDelay = 1;
                        }
                        else
                        {
                            lastOpacityDelay -= opacityDelay;
                        }
                        if(lastOpacityDelay < 0)
                        {
                            lastOpacityDelay = 0;
                    ...
                    for (int i = 0; i < opacityValue && lastOpacityDelay == 0; i++)
                    {
