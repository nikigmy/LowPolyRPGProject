...
public class LowPolyTerrainGenerator
{
	...
	void GenerateHeightmap()
	{
		...
		EditorUtility.ClearProgressBar();
		EditorUtility.SetDirty(_Terrain.Heightmap);
	}
	...
	void ReadHeightsFromRaw16()
	{
		...
		for (int y = 0; y < _Terrain.RawHeightMapSize; ++y)
		{
			...
			for (int x = 0; x < _Terrain.RawHeightMapSize; ++x)
			{
				...
				rawFloats[x, y] = (float)System.BitConverter.ToUInt16(rawBytes, index) / System.UInt16.MaxValue;
	...
	TerrainData CreateTerrainDataFromHeightmap()
	{
		...
		// Fixup scaling if the number of verts isn't a power of two
		float canonScaling = _Terrain.TerrainSize / _Terrain.Heightmap.originalQuadPerSide;
		float scaling = canonScaling / terrainData.heightmapScale.x;
		terrainData.size = new Vector3(_Terrain.TerrainSize * scaling, _Terrain.TerrainHeight, _Terrain.TerrainSize * scaling);
