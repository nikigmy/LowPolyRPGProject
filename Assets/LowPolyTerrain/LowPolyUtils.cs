...
public static class LowPolyUtils
{
	...
	public static float SampleBilinear(float[,] rawFloats, float u, float v)
	{
		...
		if (xPrev < 0 || xPrev >= rawFloats.GetLength(0) ||
			yPrev < 0 || yPrev >= rawFloats.GetLength(1) ||
			xNext < 0 || xNext >= rawFloats.GetLength(0) ||
			yNext < 0 || yNext >= rawFloats.GetLength(1))
		{
			Debug.LogError("xPrev: " + xPrev + ", yPrev: " + yPrev + ", xNext: " + xNext + ", yNext: " + yNext + ", rawFloats: " + rawFloats.GetLength(0) + ", " + rawFloats.GetLength(1));
			return 0;
		}
		else
		{
