...
namespace com.ootii.Cameras
{
    ...
    {
        public Vector2 returningCamera;
        public Vector2 cameraReturnSpeed;

        /// <summary>
        /// Determines if we use FixedUpdate or LateUpdate for our update cycle.
        /// </summary>
        public bool _UseFixedUpdate = false;
        public bool UseFixedUpdate
        {
            get { return _UseFixedUpdate; }
            set { _UseFixedUpdate = value; }
        }
        ...
        private void FollowCharacter()
        {
            ...
            {
                returningCamera = new Vector2(1, 0);
                return;
            }
            if (UnityEngine.Input.GetMouseButtonDown(0))
            {
                returningCamera = new Vector2(1, 1);
            }

            if (!UnityEngine.Input.GetMouseButton(0))
            {
                var currCameraReturnSpeed = Math.Abs(UnityEngine.Input.GetAxisRaw("Vertical")) > 0.01 ? cameraReturnSpeed * 5 : cameraReturnSpeed;

                var yrot1 = NormalizeRotation(Anchor.rotation.eulerAngles.y) - NormalizeRotation(transform.eulerAngles.y);
                var yrot2 = Anchor.rotation.eulerAngles.y - transform.eulerAngles.y;
                yrot1 = NormalizeRotation(yrot1);
                yrot2 = NormalizeRotation(yrot2);
                var yRotationDiference = Math.Abs(yrot1) <= Math.Abs(yrot2) ? yrot1 : yrot2;
                var yRotation = currCameraReturnSpeed.y * Time.deltaTime;

                //var yRotationStepsLeft = Math.Abs(Math.Round(yRotationDiference / yRotation, MidpointRounding.AwayFromZero));

                if (Mathf.Abs(yRotationDiference) > yRotation && returningCamera.y == 1)
                {
                    if (yRotationDiference < 0)
                        yRotation *= -1;
                }
                else
                {
                    yRotation = yRotationDiference;
                    returningCamera.y = 0;
                }

                var xrot1 = NormalizeRotation(Anchor.rotation.eulerAngles.x) - NormalizeRotation(transform.eulerAngles.x);
                var xrot2 = Anchor.rotation.eulerAngles.x - transform.eulerAngles.x;
                xrot1 = NormalizeRotation(xrot1);
                xrot2 = NormalizeRotation(xrot2);
                var xRotationDiference = Math.Abs(xrot1) <= Math.Abs(xrot2) ? xrot1 : xrot2;
                var xRotation = currCameraReturnSpeed.x * Time.deltaTime;

                //var xRotationStepsLeft = Math.Abs(Math.Round(xRotationDiference / xRotation, MidpointRounding.AwayFromZero));
                if (Mathf.Abs(xRotationDiference) > xRotation && returningCamera.x == 1)
                {
                    if (xRotationDiference < 0)
                        xRotation *= -1;
                }
                else
                {
                    xRotation = xRotationDiference;
                    returningCamera.x = 0;
                }
                transform.Rotate(xRotation, yRotation, 0);
                //Debug.Log("x -> Rot-" + xRotation + ", Return-" + returningCamera.x + ", Rot1-" + xrot1 + ", Rot2-" + xrot2 + ", Dif-" + xRotationDiference + " : \n" +
                //    "y -> Rot-" + yRotation + ", Return-" + returningCamera.y + ", Rot1-" + yrot1 + ", Rot2-" + yrot2 + ", Dif-" + yRotationDiference + " : \n");
            }
        }
        ...
        public static BaseCameraRig ExtractCameraRig(Transform rCamera)
        {
            ...
            while (lParent != null)
            {
                BaseCameraRig[] lRigs = lParent.gameObject.GetComponents<BaseCameraRig>();
                if (lRigs != null && lRigs.Length > 0)
                {
                    for (int i = 0; i < lRigs.Length; i++)
                    {
                        if (lRigs[i].enabled && lRigs[i].gameObject.activeSelf)
                        {
                            return lRigs[i];
                        }
                    }
